...
def check_mainview_filled(diagnosis: str, weight: Decimal) -> bool:
    if diagnosis.strip() == '':
...
class SaveBtn(wx.Button):
    ...
    def insert_visit(self):
        ...
        if check_mainview_filled(diagnosis, weight):
            ...
            try:
                with self.mv.con as con:
                    ...
                    """, (past_history,))
                    vid = con.execute(f"""
                        INSERT INTO {Visit.table_name} ({Visit.fields_as_str()})
                        VALUES ({Visit.fields_as_names()})
                    """, v).lastrowid
                    ...
                    con.executemany(f"""
                        ...
                        VALUES ({LineDrug.fields_as_names()})
                    """, insert_ld)
        ...
        if check_mainview_filled(diagnosis, weight):
            ...
            try:
                with self.mv.con as con:
                    ...
                    """, (past_history,))
                    con.execute(f"""
                        UPDATE {Visit.table_name} SET ({Visit.fields_as_str()})
